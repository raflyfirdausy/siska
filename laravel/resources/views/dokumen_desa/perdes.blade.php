...
{{-- Judul halaman --}}
@section('page-title')
    <strong>Daftar</strong> Peraturan Desa
@endsection
        ...
        <div class="row">
          ...
          <div class="col-md-5">
            <div class="pull-right">
              ...
              <a href="{{ url("/download-perdes?") }}q={{$ket['q']}}&bulan={{$ket['bulan']}}&tahun={{$ket['tahun']}}" class="btn btn-danger"><i class="fa fa-download"></i> Export</a>                   
            </div>
          </div>
        </div>
        <table width="100%" style="overflow:hidden" id="posts-table" class="table table-tools table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $s->tentang }}</td>
              <td>{{ $s->uraian }}</td>    
              <td>{{ $s->no_persetujuan }}<br>{{ $s->tgl_persetujuan }}</td>              
              <td>{{ $s->no_dilaporkan }}<br>{{ $s->tgl_dilaporkan }}</td>               
              <td>{{ $s->keterangan }}</td>
              <td>
...
<div class="modal fade" id="modal-tambah-edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="modalTambahEdit">Tambah / Edit Agenda Peraturan Desa</h4>
      </div>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input required type="text" id="no_persetujuan" name="no_persetujuan" class="form-control">                                    
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">Tanggal Persetujuan</label>
                    <input required type="date" id="tgl_persetujuan" name="tgl_persetujuan" class="form-control">                             
                </div>
              </div>
            </div>    
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">Nomor Dilaporkan</label>
                    <input required type="text" id="no_dilaporkan" name="no_dilaporkan" class="form-control">                                    
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">Tanggal Dilaporkan</label>
                    <input required type="date" id="tgl_dilaporkan" name="tgl_dilaporkan" class="form-control">                             
                </div>
              </div>
            </div>                    
            <div class="row">
  ...
  <script>
    ...
    //   $("#NomerWarning").text("");
      $("#form_tambah_edit").attr("action", '{{ URL::to("/tambah-perdes") }}'); 
    });

    $('.editData').on('click', function() {
      $("#form_tambah_edit").attr("action", '{{ URL::to("/edit-perdes") }}');
      $("#NomerWarning").text("");
      ...
      $('#no_persetujuan').val(surat.no_persetujuan);
      $('#tgl_persetujuan').val(surat.tgl_persetujuan);
      $('#no_dilaporkan').val(surat.no_dilaporkan);
      $('#tgl_dilaporkan').val(surat.tgl_dilaporkan);        
    ...
    $('.hapusData').on('click', function() {
      ...
      $('#form-delete').attr('action', '{{ URL::to("/hapus-perdes") }}');
    });    
